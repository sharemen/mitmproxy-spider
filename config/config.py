...
if not os.path.exists('config-default.yaml'):
    if not os.path.exists(""):
        os.mkdir("")
    shutil.copyfile('config.yaml', './config/config-default.yaml')

config = yaml.full_load(open(abs_path('./config/config-default.yaml'), encoding='utf8'))
"""
config_path = './config.yaml'
#config_path = './config_test.yaml'

if not os.path.exists(abs_path(config_path)):
    file = open(abs_path(config_path), 'w')
    file.close()
    shutil.copyfile(abs_path('./config-default.yaml'),abs_path(config_path))

config = yaml.full_load(open(abs_path(config_path), encoding='utf8'))
