...
def deal_json_invaild(data):
    ...
    data = data.replace('"', '”')\
        ...
        .replace('&&testPassword2&&','":').replace("$$testPassword2$$",',"')
    #print(data)
    return data
...
def get_json(json_str):
    ...
    try:
        ...
        try:
            json_str2 = json_str.strip()
            json_str2 = deal_json_invaild(json_str2)
            return json.loads(json_str2, strict=False) if json_str2 else {}
...
def make_insert_sql(table, data, auto_update=False, update_columns=(), insert_ignore=False):
    '''
    @summary: 适用于mysql， oracle数据库时间需要to_date 处理
    ---------
    @param table:
